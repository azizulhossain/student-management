...
def update(id, name=" ", fname=" ", mname=" ", address=" ", mobno=" ", email=" ", dob=" ", gender=" "):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE student SET name = ? OR fname = ? OR mname = ? OR address = ? OR mobno = ? OR email = ? OR dob = ? OR "
        "gender = ?",
           (name, fname, mname, address, mobno, email, dob, gender))
    conn.commit()
    conn.close()
...
def search(name=" ", fname=" ", mname=" ", address=" ", mobno=" ", email=" ", dob=" ", gender=" "):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM student WHERE name = ? OR fname = ? OR mname = ? OR address = ? OR mobno = ? OR email = ? OR "
        "dob = ? OR gender = ?",
        (name, fname, mname, address, mobno, email, dob, gender))
    rows = cur.fetchall()
    return rows
